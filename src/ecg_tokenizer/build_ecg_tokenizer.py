...
class BuildECGByte(BuildECGTokenizers):
    ...
    def train_ecg_byte(self):
        ...
        output_path = f"src/ecg_tokenizer/ecg_byte_tokenizer_{self.args.num_merges}.pkl"
    ...
    def verify_ecg_byte(self):
        ...
        ecg_path = random.choice(file_paths).strip()

        ecg_signal = np.load(ecg_path, allow_pickle=True).item()["ecg"]
    ...
    def mp_process_ecg_to_symbol(self):
        ...
        if getattr(self.args, "dev", False):
            num_samples = min(25, len(file_paths))
